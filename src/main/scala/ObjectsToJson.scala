...
/**
  * Contains utility function to convert output data to JSON
  */
object ObjectsToJson {
  ...
  def success(comment: Option[String] = Option.empty) = {
    if (comment.isEmpty) pretty(render("success" -> true))
    else pretty(render(("success" -> true) ~ ("comment" -> comment)))
  }
  ...
  private def operationPack(oper: QueuedOperation):JValue = oper match {
